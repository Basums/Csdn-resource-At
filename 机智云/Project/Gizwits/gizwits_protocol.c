...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagLED_Switch)
    {
        bitFieldBitLen += LED_Switch_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagLED_Switch)
    {
        info->event[info->num] = EVENT_LED_Switch;
        info->num++;
        dataPoints->valueLED_Switch = gizVarlenDecompressionValue(bitFieldOffset,LED_Switch_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += LED_Switch_LEN;
    }


    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtemperature_UPX)
    {
        info->event[info->num] = EVENT_temperature_UPX;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetemperature_UPX,(uint8_t *)&issuedData[byteFieldOffset],temperature_UPX_LEN);
        byteFieldOffset += temperature_UPX_LEN;
        dataPoints->valuetemperature_UPX = gizX2YFloat(temperature_UPX_RATIO,  temperature_UPX_ADDITION, (float)(exchangeBytes(elongateP0FormTmp.devStatus.valuetemperature_UPX)));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLED_Switch != cur->valueLED_Switch)
    {
        GIZWITS_LOG("valueLED_Switch Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagLED_Switch = 1;
        ret = 1;
    }
    if(last->valuetemperature_UPX != cur->valuetemperature_UPX)
    {
        GIZWITS_LOG("valuetemperature_UPX Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtemperature_UPX = 1;
        ret = 1;
    }

    if(last->valuetemperature_Gizwits != cur->valuetemperature_Gizwits)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuetemperature_Gizwits Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagtemperature_Gizwits = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    /*** Fill the bit ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flagLED_Switch)
    {
        gizVarlenCompressValue(bitFieldOffset,LED_Switch_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueLED_Switch);
        bitFieldOffset += LED_Switch_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagtemperature_UPX)
    {
        devStatusTmp.valuetemperature_UPX = exchangeBytes(gizY2XFloat(temperature_UPX_RATIO,  temperature_UPX_ADDITION, dataPoints->valuetemperature_UPX));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetemperature_UPX,temperature_UPX_LEN);
        byteFieldOffset += temperature_UPX_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagtemperature_Gizwits)
    {
        devStatusTmp.valuetemperature_Gizwits = exchangeBytes(gizY2XFloat(temperature_Gizwits_RATIO,  temperature_Gizwits_ADDITION, dataPoints->valuetemperature_Gizwits));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetemperature_Gizwits,temperature_Gizwits_LEN);
        byteFieldOffset += temperature_Gizwits_LEN;
    }
