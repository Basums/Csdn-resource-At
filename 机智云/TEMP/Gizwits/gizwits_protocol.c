...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagkaiguan)
    {
        info->event[info->num] = EVENT_kaiguan;
        info->num++;
        dataPoints->valuekaiguan = gizStandardDecompressionValue(kaiguan_BYTEOFFSET,kaiguan_BITOFFSET,kaiguan_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuekaiguan != cur->valuekaiguan)
    {
        GIZWITS_LOG("valuekaiguan Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(kaiguan_BYTEOFFSET,kaiguan_BITOFFSET,kaiguan_LEN,(uint8_t *)devStatusPtr,dataPoints->valuekaiguan);
