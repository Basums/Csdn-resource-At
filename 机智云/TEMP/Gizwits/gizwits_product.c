...
int8_t gizwitsEventProcess(eventInfo_t *info, uint8_t *gizdata, uint32_t len)
{
  ...
  for(i=0; i<info->num; i++)
  {
    ...
    {
      case EVENT_kaiguan:
        currentDataPoint.valuekaiguan = dataPointPtr->valuekaiguan;
        GIZWITS_LOG("Evt: EVENT_kaiguan %d \n", currentDataPoint.valuekaiguan);
        if(0x01 == currentDataPoint.valuekaiguan)
        {
          //user handle
						keyOnorOff = 0;
...
void userHandle(void)
{
 /*
    ...
    */
	...
	if(keyOnorOff == 0)
		currentDataPoint.valuekaiguan = 1;
	else
		currentDataPoint.valuekaiguan = 0;
}
